"use client";
import { useGetItemsCode } from "@/lib/hooks/use-get-items-code";
...
function ItemsCodeBlock() {
  ...
  const { data, isLoading } = useGetItemsCode(items, mode);
          ...
          {data && !isLoading && (
            <CodeBlock code={data ?? ""} language="typescript">
              <ScrollArea className="relative size-full">
                <CodeBlock.Code className="bg-black/90 p-4 text-sm leading-6 tracking-wide dark:bg-black/50">
...
function CodeBlockHeader() {
  ...
  const { data } = useGetItemsCode(items, mode);
