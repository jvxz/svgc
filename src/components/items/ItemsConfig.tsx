...
import { setItemOptions } from "@/lib/store/items";
...
function ItemsConfig() {
  ...
  const handleCheckedChange = (checked: boolean) => {
    ...
    setItemOptions(targetItem.data, { persistBrandColors: checked });
  };

  return (
    <div className="col-span-2 size-full border-r border-border">
      <div className="flex flex-wrap gap-2 p-4">
        <div className="relative flex w-1/2 items-start gap-2 rounded-lg border border-input p-4 shadow-sm shadow-black/5 has-[[data-state=checked]]:border-ring">
          <Switch
            id={id}
            ...
            checked={targetItem?.options.persistBrandColors ?? false}
          />
          <div className="flex grow items-start gap-3 peer-data-[state=unchecked]:[&_svg]:grayscale">
            <svg
              className="shrink-0"
              width={32}
              height={24}
            ...
            <div className="grid grow gap-2">
              <Label htmlFor={id}>Persist brand colors</Label>
              <p
                id={`${id}-description`}
                className="text-xs text-muted-foreground"
              >
                Keep brand logos persistent regardless of text color
              </p>
            </div>
          </div>
        </div>
